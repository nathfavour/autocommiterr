...
async function performCommit(options: any = {}): Promise<void> {
  try {
    ...
    if (!quiet) console.log('📍 Repository:', cwd);

    // 1) Ensure .gitignore safety
    if (!quiet) console.log('🔒 Ensuring .gitignore safety...');
    await gitUtils.ensureGitignoreSafety(cwd);
    if (!quiet) console.log('✓ .gitignore is safe.');

    // 2) Stage all changes
    if (!quiet) console.log('📦 Staging changes (git add .)...');
    gitUtils.stageAllChanges(cwd);

    // 3) Check for staged files
    if (!quiet) console.log('🔍 Checking staged changes...');
    const staged = gitUtils.getStagedFiles(cwd);
    if (!staged) {
      console.log('ℹ️  No changes to commit — Autocommit skipped.');
      return;
    }
    ...
    if (!message) {
      if (!quiet) console.log('🤖 Generating commit message...');
      
      const apiKey = await modelManager.getApiKey();
      if (!apiKey) {
        if (!quiet) console.log('⚠️  No API key found. Using default message.');
        ...
        try {
          ...
          try {
            ...
            if (aiResult && aiResult.trim().length > 0) {
              ...
              if (!quiet) console.log(`✓ Used ${selectedModel} to generate the message.`);
            }
          } catch (apiErr) {
            if (!quiet) console.error('⚠️  API-based generation failed:', (apiErr as Error)?.message);
            message = 'chore: automated commit generated by Autocommiter';
          }
        } catch (e) {
          if (!quiet) console.error('⚠️  Failed to generate message:', (e as Error)?.message);
          message = 'chore: automated commit generated by Autocommiter';
        }
      }
    }
    ...
    // 6) Commit changes
    if (!quiet) console.log('💾 Committing staged changes...');
    await gitUtils.commit(cwd, message);
    console.log('✓ Changes committed successfully.');

    // 7) Push if requested
    if (options.push) {
      if (!quiet) console.log('🚀 Pushing to remote...');
      gitUtils.push(cwd);
...
program
  ...
  .action(async () => {
    try {
      ...
      if (models.length > 0) {
        await modelManager.updateCachedModels(models);
        console.log(`✓ Successfully fetched and cached ${models.length} models.`);
      } else {
